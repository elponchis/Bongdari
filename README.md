...
- **한도가 조용히 소진되지 않게** 앱이 사용량을 세어 보여줍니다. 헤더 오른쪽에
  `오늘 N / 10,000 뉴런` 이 표시되고, 50%에서 노랑·80%에서 빨강으로 바뀌며
  경고 배너가 뜹니다. 한도를 실제로 넘기면 화면 상단에 빨간 배너로
  "오전 9시 초기화 / 유료 전환" 안내가 남습니다.
  단, 이 눈금은 **localStorage 기반이라 이 브라우저 몫만** 셉니다. 여러 사람이
  쓰면 계정 합계는 더 크므로, 정확한 값은 Cloudflare 대시보드에서 확인하세요.
  (Cloudflare 는 Workers AI 사용량 알림을 제공하지 않아 앱에서 대신 셉니다.)
