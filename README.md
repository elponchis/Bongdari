...
**암호는 선택 사항입니다.** Environment Variables 에 `CLUB_PASSWORD` 를 넣으면
그 값을 아는 사람만 단어장을 읽고 쓸 수 있고, 넣지 않으면 주소를 아는 사람
누구나 쓸 수 있습니다. 앱이 서버에 물어보고 알아서 암호 칸을 보이거나 숨깁니다.
나중에 환경변수만 추가하면 코드 변경 없이 잠깁니다.
...
- 배포 주소를 아는 사람은 누구나 `/api/explain`을 부를 수 있습니다.
  ...
  `CLUB_PASSWORD` 를 설정한 경우에만 잠깁니다 — 안 넣으면 열려 있습니다.
